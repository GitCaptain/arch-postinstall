...
GPU_CONFIG="$TARGET_DIR/gpu.conf"
...
install_generated_gpu_config() {
  ...
  {
    printf '# Generated by arch-postinstall.\n'
    printf '# Primary GPU policy: %s\n' "$GPU_PRIMARY"
    if [[ -n "$devices" ]]; then
      printf 'env = AQ_DRM_DEVICES,%s\n' "$devices"
    else
      printf '# AQ_DRM_DEVICES intentionally left unset: auto policy on a multi-GPU system.\n'
      printf '# Re-run with --gpu-primary nvidia|amd|intel for an explicit renderer.\n'
    fi
  } > "$tmp"
